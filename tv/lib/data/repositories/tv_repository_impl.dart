...
class TvRepositoryImpl implements TvRepository {
  ...
  @override
  Future<Either<Failure, List<Tv>>> getShowingTodayTvs() async {
    try {
      final result = await remoteDataSource.getShowingTodayTvs();
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return const Left(ServerFailure(''));
    } on SocketException {
  ...
  @override
  Future<Either<Failure, List<Tv>>> getUpcomingTvs() async {
    try {
      final result = await remoteDataSource.getUpcomingTvs();
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return const Left(ServerFailure(''));
    } on SocketException {
